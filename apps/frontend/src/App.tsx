...
import { createVoiceSession, type VoiceSessionResponse } from "./api";
...
type RouteId = "explore" | "concierge";
...
function getRouteState(): RouteState {
  ...
  const routeId = pathname.startsWith("/concierge") ? "concierge" : "explore";
  const isCallRoute =
    pathname === "/explore/call" || pathname === "/concierge/call";
...
function VoiceExperience() {
  ...
  const [selectedCallAgent, setSelectedCallAgent] = useState<AgentId>(() =>
    getDefaultAgentForRoute(getRouteState().routeId),
  );
  const [selectedEngine, setSelectedEngine] =
    useState<DemoEngineId>("elevenlabs");
  const [toolCallCount, setToolCallCount] = useState(0);
  const [voiceSession, setVoiceSession] = useState<VoiceSessionResponse>();
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      ...
      setSelectedCallAgent(getDefaultAgentForRoute(nextRouteState.routeId));
  ...
  function handleOpenCallScreen(page: PageConfig) {
    setSelectedCallAgent(page.agentId);
    navigate(`/${routeState.routeId}/call`);
  }
          ...
          <>
            ...
            <div className="hero-content">
              ...
              <button
                ...
                type="button"
                onClick={() => handleOpenCallScreen(page)}
              >
                {phoneIcon()}
                {page.ctaLabel}
              </button>
